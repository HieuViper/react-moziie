...
import TVRecommend from "../components/recommendation/TVRecommend";
...
const TVWatch = () => {
  ...
  const { data, error } = useSWR(
    `https://api.themoviedb.org/3/tv/${state}?api_key=${API_THEMOVIEDB}`,
    fetcher
  );
  if (!data) return null;
  ...
  return (
    <div className="page-container flex gap-10">
      <div className="basis-9/12 h-screen flex flex-col gap-4">
        <iframe
          src={`https://2embed.org/embed/series?tmdb=${state}`}
          width="100%"
          height="500"
          scrolling="no"
          frameborder="0"
        ...
        <div className="flex gap-3">
          ...
          <div className="flex items-center gap-1 ">
            ...
            <p className="text-gray-400">{first_air_date}</p>
          </div>
        </div>
